...
const JackpotModal = ({ jackpotKey, onClose }) => {
  ...
  const { jackpot } = socketCtx || {};
  const { formatCurrency, user, wallet } = useUser();
  ...
  // Listen to jackpot events
  useEffect(() => {
    if (!socket) return;
    const onConfirmed = ({ ticketRef, jackpotName, stake }) => {
      setSubmitting(false);
      setMsg({ type: 'success', text: `✅ ${jackpotName} ticket submitted! Ref: ${ticketRef} | Stake: KSh ${stake}` });
    ...
    socket.on('jackpot_stake_confirmed', onConfirmed);
    socket.on('jackpot_error', onError);
    return () => {
      socket.off('jackpot_stake_confirmed', onConfirmed);
      socket.off('jackpot_error', onError);
    };
  }, [socket, onClose]);
  ...
  const handleSelect = (gameId, type) => {
    setSelections(prev => {
      const next = { ...prev };
      if (next[gameId] === type) delete next[gameId];
  ...
  const minStake = currentJackpot.minStake || 99;

  const handlePlaceStake = () => {
    if (!user) {
      setMsg({ type: 'error', text: '❌ Please register or log in to place a jackpot stake.' });
      setTimeout(() => setMsg(null), 4000);
      return;
    }
    if (wallet < minStake) {
      setMsg({ type: 'error', text: `❌ Insufficient balance. Min stake is KSh ${minStake}.` });
      setTimeout(() => setMsg(null), 4000);
      return;
    }
    if (!socket) return;
    setSubmitting(true);
    socket.emit('place_jackpot_stake', {
      jackpotKey,
      jackpotName: currentJackpot.name,
      selections,
      games: games.map(g => ({ id: g.id, home: g.home, away: g.away, odds: g.odds })),
    });
  };
      ...
      <div className="animate-enter" style={{
        ...
        <div style={{
          ...
          <div>
            ...
            <div style={{ color: 'var(--text-muted)', fontSize: '13px', marginTop: '4px' }}>
              ...
              {' · '}Min stake: <span style={{ color: currentJackpot.color || 'var(--primary)', fontWeight: 700 }}>KSh {minStake}</span>
            </div>
          </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '1.5rem' }}>
          {msg && (
            <div style={{
              ...
              backgroundColor: msg.type === 'success' ? 'rgba(40,167,69,0.15)' : 'rgba(220,53,69,0.15)',
        ...
        <div style={{
          ...
          <div style={{ textAlign: 'right' }}>
            ...
            <div style={{ fontSize: '16px', fontWeight: 700, color: 'var(--primary)' }}>{formatCurrency(wallet)}</div>
          </div>
          <button
            ...
            style={{
              ...
              backgroundColor: allSelected && !submitting ? (currentJackpot.color || 'var(--primary)') : 'var(--bg-btn)',
              color: allSelected && !submitting ? '#000' : 'var(--text-muted)',
              cursor: allSelected && !submitting ? 'pointer' : 'not-allowed',
              border: 'none', minWidth: '200px'
            }}
            onClick={handlePlaceStake}
          >
            {submitting ? '⏳ Submitting...' : allSelected ? `Place ${currentJackpot.name} Stake` : 'Select all matches'}
          </button>
        </div>
      </div>
    </div>
